...
public class NiconSystemAdmin{
  ...
  public static boolean VerifyStateSystem(){
    try{
      int StateLibrary=NiconAdministrator.VerifyAdministratorState();
      if (StateLibrary==0) {
         StateActivationSystem=true;    
      }
    } catch (Exception e) {
      JOptionPane.showMessageDialog(null, "ERROR  en VerifyStateSystem() \n" + e, "Nicon v 0.1 SystemAdmin", 0);
    }
    return StateActivationSystem;
  }
  ...
  public static boolean VerifyIdentityUser(String InputPassword){
    boolean acces = false;
    try {
      String FormatPassword=EncryptationSecurityService(InputPassword);
      sentence = "select Password from administrador;";
      Data = NiConection.RestoreDataSentence(sentence);
      Data.next();
      String GetPassword = Data.getString("Password");
      Data.close();
      NiConection.DisconectDB();
      if ((GetPassword.equals(FormatPassword))) {
        acces = true;
        sentence = null;
        FormatPassword=null;
        InputPassword=null;
